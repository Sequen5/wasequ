...
/******** OWNER NUMBER**********/
const ownerNumber = ["6289689542663@s.whatsapp.net","6282334297175@s.whatsapp.net"] 
/************************************/
               ...
               if (data.error) return reply(data.error)
				...
				case 'bcgc':
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Ini adalah list group Sequen BOT :\n\n\`\`\``
					no = 0
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					reply(`Prefix berhasil diubah : ${prefix}`)
					break 
				case 'tiktokstalk':
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						if (args.length < 1) return client.sendMessage(from, 'user name ?', text, {quoted: mek})
           ...
           case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝘆𝗮𝗵𝗵 @${mentioned[0].split('@')[0]} jabatan admin kamu sudah dicopot𝘁🏃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Selamat🥳 anda naik menjadi admin group (*_*) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
					...
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Maap ya ,otw kick 🏃 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupRemove(from, mentioned)
					} else {
						mentions(`Maap ya ,otw 𝗸ick! @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				if (!isQuotedSticker) return reply('𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 𝘀𝘁𝗶𝗰𝗸𝗲𝗿 !')
					...
					break
                 case 'simi':
					if (args.length < 1) return reply('Textnya mana kakak :( ?')
					teks = body.slice(5)
				...
				case 'simih':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Boo, Sudah aktif ya :𝘃')
					if (Number(args[0]) === 1) {
                ...
                if (!isGroup) return reply(ind.groupo())
                if (!isGroupAdmins) return reply(ind.admin())
                if (args.length < 1) return reply('Boo, Sudah aktif ya :??')
                if (args[0] === 'enable') {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ SUKSES ❭ Menonaktifkan fitur walcome/left di group ini')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*❬ SUKSES ❭ Mengaktifkan EVENT di group ini*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ SUKSES ❭ Menonaktifkan EVENT di group ini*')
					} else {
						reply(ind.satukos())
					}
					break
